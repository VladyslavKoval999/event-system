...
import { registerParticipant } from '../api/index';
...
const schema = z.object({
    ...
    source: z.enum(["social", "friends", "ads"], { errorMap: () => ({ message: "Оберіть варіант" }) })
});
...
const RegistrationPage = () => {
    ...
    const onSubmit = async (data) => {
        try {
            await registerParticipant({ ...data, eventId });
            alert("Ви успішно зареєстровані!");
            navigate(`/participants/${eventId}`);
        } catch (err) {
            console.error("Помилка реєстрації:", err);
            alert("Помилка реєстрації. Можливо, потрібна авторизація.");
        }
    };
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="form-group">
                        ...
                        <div className="radio-group">
                            <label className="radio-label"><input type="radio" value="social" {...register("source")} /> Соцмережі</label>
                            <label className="radio-label"><input type="radio" value="friends" {...register("source")} /> Друзі</label>
                            <label className="radio-label"><input type="radio" value="ads" {...register("source")} /> Реклама</label>
                        </div>
